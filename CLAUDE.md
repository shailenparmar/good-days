...
**Paste validation (v2.1.32+, updated v2.3.10):** When pasted clipboard content doesn't match any supported format, the copy|paste split is replaced by a single full-width "invalid format" indicator for 1.5 seconds, then reverts to copy|paste. Mobile: rendered as a full-width button (same `getButtonStyle` with position `'full'`), dismisses on any click or keystroke. Desktop: rendered as a full-width `<div>` with matching border styling, dismisses when the mouse leaves the color stats hover region (the same hitbox that activates copy/paste buttons). No colors are applied. State: `pasteInvalid` boolean + `pasteInvalidTimer` ref. Works on both mobile (`main.tsx`) and desktop (`StatsDisplay.tsx`).
