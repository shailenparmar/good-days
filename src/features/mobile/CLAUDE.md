...
**`sendColorThrottled` helper (v2.4.29+, updated v2.4.124):** A `useCallback` that wraps the WS send with 83ms throttle (~12fps). Used by both `processTouchAt` (hue changes) and the orientation handler (sat/light changes). Replaces 4 identical inline throttle blocks. Was 42ms (~24fps) in v2.4.123, 33ms (~30fps) before that, 21ms (~48fps) before v2.4.120.
