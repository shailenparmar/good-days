...
export function MobileButton({ onActivate, children, getStyle, extraProps }: MobileButtonProps) {
  ...
  return (
    <div
      data-btn
      onTouchStart={(e) => { e.preventDefault(); engaged.current = true; setPressed(true); if (navigator.vibrate) navigator.vibrate(10); }}
      onTouchMove={(e) => {
        const inside = isTouchInside(e);
        engaged.current = inside;
        setPressed(inside);
        if (inside && navigator.vibrate) navigator.vibrate(5);
      }}
