...
6. **Rubber band suppressed (v2.6.64+):** `overscrollBehaviorY: 'none'` on the textarea when scrambled. Native rubber banding causes the textarea to visually overscroll while `scrollTop` stays clamped — the overlay can't follow because there's no JS API for the rubber band offset. Suppressing it means scroll stops hard at the boundary, keeping overlay and textarea in perfect sync. When not scrambled, the inline style is `undefined` so `.scrollbar-hide`'s `contain` takes over normally.
