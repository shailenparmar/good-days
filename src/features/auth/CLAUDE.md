...
**Progressive entry loading (v2.7.23+):** `initJournalStorage` accepts an optional `onProgress` callback. When provided, entries decrypt one at a time (sequential `for` loop instead of `Promise.all`), calling `onProgress` with the cumulative sorted array. **Note (v3.1.0+):** This path is no longer used by the unlock flow — `useJournalEntries` now calls `loadEntryIndex()` instead. The progressive callback is preserved on `initJournalStorage` for legacy paths and optional re-introduction.

**Batched emits + single sort (v3.0.9+):** When the progressive path is used, `getEntriesFromIndexedDB` only emits every 25 entries or 50ms (whichever first), and sorts once at the end of the loop. Constants: `BATCH_SIZE = 25`, `BATCH_MS = 50`. Superseded by the v3.1.0 two-phase architecture for the unlock hot path; kept for the rare progressive-decrypt path.

**Two-phase load architecture (v3.1.0+):** Unlock no longer decrypts every entry before the editor is usable. Previously a user with 1000+ entries would wait 20+ seconds because the sidebar render gated on full-corpus decrypt + state hydration. The decoupling: an entry's *index* (date, startedAt, lastModified) is plaintext on the encrypted record; only `content` and `title` need decryption.

- **Phase A — `loadEntryIndex()` in `journalStorage.ts`:** Reads all records from IDB, returns `{ entries, encryptedDates }`. For encrypted records, returns a stub `{ date, startedAt, lastModified, content: '', title: undefined }` and adds the date to `encryptedDates`. Legacy plaintext entries are returned in full and are NOT in the set. Fast path: no AES-GCM. Falls back to `initJournalStorage()` for migrations or dead-mans-switch.
- **Phase B — decrypt today + selectedDate:** `useJournalEntries` calls `loadSingleEntry()` for `getTodayDate()` and the persisted `selectedDate` (deduped) so the editor is writable within ~10ms of Phase A completing.
- **Lazy decrypt on navigation:** The `selectedDate` effect checks `loadedDatesRef.current.has(date)`; if not, calls `loadSingleEntry`, patches the entry into state, and updates `currentContent`. Logged as `journal.entry.lazyLoaded` with `durationMs`.
- **Safety guards:** `loadedDatesRef: useRef<Set<string>>` tracks which dates have full content. `saveEntry` and `saveTitle` (when an entry already exists) refuse to write if the date isn't loaded — without this, typing on a not-yet-decrypted past entry would wipe its ciphertext on first keystroke. Skipped saves are logged as `journal.save.skipped` / `journal.saveTitle.skipped`. Brand-new title-only entries (no `existingIndex`) and the in-memory "ensure today" placeholder (no ciphertext exists yet) are added to `loadedDatesRef` immediately.
- **`reloadEntries()` (re-lock + unlock path):** Mirrors the two-phase flow. Clears `loadedDatesRef` first.
- **Multi-tab `onEntrySaved`:** Adds the patched date to `loadedDatesRef` since it just got fresh content from another tab.
- **Action log:** `journal.index.loaded` (Phase A duration), `journal.loaded` (total to interactive), `journal.entry.lazyLoaded` (per-entry lazy decrypt).

Code locations: `useAuth.ts` (`handlePasswordSubmit`), `LockScreen.tsx` (`handleSubmit`), `App.tsx` (`handlePasswordSubmit`), `journalStorage.ts` (`initJournalStorage`, `getEntriesFromIndexedDB`, `loadEntryIndex`, `loadSingleEntry`), `useJournalEntries.ts` (Phase A + B load effect, `selectedDate` lazy-decrypt effect, `loadedDatesRef`, `reloadEntries`, `journal.loaded` / `journal.index.loaded` / `journal.entry.lazyLoaded` logs)

### Set-Password Auto-Focus (v3.1.1+)

Previously `PasswordSettings` intentionally did NOT auto-focus the input when settings opened with no password — the rationale was "typing should go to the editor." But this created a footgun: the user would see the "set password" placeholder, type, watch their password go silently into the journal editor, press Enter, and nothing would happen because the form's input was empty (`if (!input.trim()) return;` early-returned).

Now there's a `useEffect` in `PasswordSettings.tsx` that focuses the input when `!hasPassword && showInput && !isSaving && step === 'set'` — i.e., the set-password flow is the active state. Auto-focus uses `requestAnimationFrame` so the focus lands after React commits. The "change password" flow already auto-focused via `handleChangePasswordClick`; this brings parity for the no-password case.

`flashRed` in `PasswordSettings.tsx` was also bumped from 3 short pulses (400ms total, 80ms each) to 4 longer pulses (600ms total, first held 150ms) — matching the `LockScreen` flashRed update from v3.0.10. Short pulses on a stressed render thread were getting coalesced invisibly.
