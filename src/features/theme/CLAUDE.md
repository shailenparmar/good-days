...
**SL gradient freeze during streaming (v2.5.3+, fixed v2.5.4):** The SL picker gradients conditionally use CSS vars or React state for their background hue based on `isLiveStreaming`. When NOT streaming: CSS vars (`var(--h)`/`var(--bh)`) — gradient updates live during local hue drags via ThemeContext's CSS var sync effect. When streaming: React state (`hue`/`bgHue`) — gradient freezes because WebSyncBridge writes CSS vars directly without updating React state, eliminating expensive gradient bitmap regeneration ~40x/sec. The indicator dots still move smoothly via CSS vars in both modes. On stream-stop, `isLiveStreaming` flips false → component switches back to CSS vars → gradient shows correct final hue. This was the cause of indicator stutter when settings was open during pair mode.
