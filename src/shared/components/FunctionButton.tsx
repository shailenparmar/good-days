...
export function FunctionButton({ onClick, disabled, isActive, children, dataAttribute, size = 'default', ariaLabel, title }: FunctionButtonProps) {
  ...
  return (
    <button
      ...
      title={title}
      className={`w-full px-3 py-2 font-mono rounded disabled:opacity-50 flex items-center justify-center gap-2 outline-none focus:outline-none select-none ${size === 'sm' ? 'text-xs font-bold' : 'font-extrabold'}`}
      style={{
